...
namespace Library.DAL
{
    public class MemberDAL
    {
        ...
        public List<Member> GetMembers()
        {
            ...
            {
                Member m = new Member();
                m.Id = Convert.ToInt32( reader["MemberId"].ToString());
                m.Name = reader["Name"].ToString();
                m.FatherName = reader["FatherName"].ToString();
                m.Category = reader["Category"].ToString();
                m.Type = (MemberType)Convert.ToInt32(reader["MemberType"].ToString());
                temp.Add(m);
            }
            reader.Close();
            con.Close();
